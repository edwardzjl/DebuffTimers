...
function SetActionRank(name, rank)
	local _, _, rank = strfind(rank or '', 'Уровень (%d+)')
...
do
	...
	do
		...
		function _G.CastSpellByName(text, onself)
			if not onself then
				local name = text
				local rank = 1
				for a,b in string.gfind(text, "(.-)%(Уровень (%d+)%)") do
					name = a
					rank = b
				end
				casting[name] = TARGET
	...
	function CHAT_MSG_SPELL_FAILED_LOCALPLAYER()
		for action in string.gfind(arg1, 'Вам не удалось %a+ (.*):.*') do
			casting[action] = nil
		end
	end
	...
	do
		local patterns = {
			'обладает невосприимчивостью к вашему заклинанию \"(.*)\".%.',
			'Ваше заклинание (.*) не попадает по',
			'сопротивляется вашему заклинанию \"(.*)\".%.',
			'Ваше заклинание (.*) не достигает',
			'уклоняется от вашего заклинания \"(.*)\".%.',
			'отклоняет ваше заклинание \"(.*)\".%.',
			'отражает ваше заклинание \"(.*)\".%.',
			'парирует вашу способность \"(.*)\".%.',
		}
...
function CHAT_MSG_SPELL_AURA_GONE_OTHER()
	for effect, unit in string.gfind(arg1, 'Действие эффекта (.+) наложенного на (.+) заканчивается%.') do
		AuraGone(unit, effect)
	end
end

function CHAT_MSG_SPELL_BREAK_AURA()
	for unit, effect in string.gfind(arg1, "(.+) теряет (.+)%.") do
		AuraGone(unit, effect)
	end
end
...
function CHAT_MSG_COMBAT_HOSTILE_DEATH()
	for unit in string.gfind(arg1, '(.+) погибает') do -- TODO does not work when xp is gained
		if IsPlayer(unit) then
			UnitDied(unit)
...
function CHAT_MSG_COMBAT_HONOR_GAIN()
	for unit in string.gfind(arg1, '(.+) погибает') do
		UnitDied(unit)
	end
end
...
do
	...
	elseif class == "WARRIOR" then
		bonuses = {
			["Деморализующий крик"] = function()
				return rank(2, 1) * 3
			end,
		}
	elseif class == 'HUNTER' then
		bonuses = {
			["Замораживающая ловушка"] = function(t)
				return t * rank(3, 7) * .15
			end,
			["Ледяная ловушка"] = function(t)
				return t * rank(3, 7) * .15
			end,
		}
	...
	elseif class == 'MAGE' then
		bonuses = {
			["Конус холода"] = function()
				return min(1, rank(3, 2)) * .5 + rank(3, 2) * .5
			end,
			["Ледяная стрела"] = function()
				return min(1, rank(3, 2)) * .5 + rank(3, 2) * .5
			end,
			["Превращение"] = function()
			...
			if not timer.DR and AUF.UnitName("target") == timer.UNIT then
				for i=1,16 do
					if UnitDebuffText("target",i) == timer.EFFECT and getglobal(AUF.DebuffAnchor..i) and not AUF.DoubleCheck[timer.EFFECT] then
						if timer.EFFECT == "Устрашающий крик" then -- exception
						else
...
function AUF:BuildOptions()
	...
	AUF.Options.Headline:SetText("Опции DebuffTimers")
	...
	AUF.Options.DebuffFont:SetText("Плказывать Дебафы:")
	...
	AUF.Options.SizeFont:SetText("Размер шрифта таймера:")
	...
	AUF.Options.OkButton:SetText("Сохранить")
...
function AUF:SaveOptions()
	...
	DEFAULT_CHAT_FRAME:AddMessage("|cFF00FF00Debuff Timers:|r Настройки сохранены.|r",1,1,1)
	AUF.Options:Hide()
end
...
function AUF:DatabasePreload()
	if CLASS == "MAGE" then
		...
		if rank == 3 then
			AUF_Debuff["MAGE"].EFFECT["Критическая масса"] = {
				ICON = "Spell_Fire_SoulBurn",
				DURATION = 30,
			}
			
			AUF_Debuff["MAGE"].SPELL["Ожог"] = {
				DURATION = {30, 30, 30, 30, 30, 30, 30},
				EFFECT = "Критическая масса",
			}
